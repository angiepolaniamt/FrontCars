function autoInicioRelacionCliente(){
    ...
    $.ajax({
        ...
        success:function(respuesta){
            ...
            $select.append('<option value="" hidden>Seleccionar Cliente</option>');
...
function autoInicioCar(){
    ...
    $.ajax({
        ...
        success:function(respuesta){
            ...
            $select.append('<option value="" hidden>Seleccionar Nombre</option>');
            $.each(respuesta, function (id, name) {
...
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://129.151.112.106:8080/api/Reservation/save",
...
function listarReservation(){
    $.ajax({
        url:"http://129.151.112.106:8080/api/Reservation/all",
...
function pintarRespuestaReservation(response){
    ...
    myTable+="<tr>";
        ...
        myTable+="<td>"+response[i].car.name+"</td>";
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.112.106:8080/api/Reservation/" + idElemento,
            type: 'DELETE',
            contentType: "application/JSON",
            success: function (response) {
                console.log(response);
...
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.112.106:8080/api/Reservation/"+id,
...
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.112.106:8080/api/Reservation/update",
